...
class data_partition():
    ...
    def load_data(self):
        ...
        #########
        self.gnss_paths = read_gnss_from_text('{}{}/{}.txt'.format(gnss_dir, self.folder, self.folder))
        #########
        ...
        self.img_paths_list, self.poses_list, self.imus_list, self.gnss_list = [], [], [], []
        start = 0
        ...
        while start + self.seq_len < n_frames:
            ...
            self.gnss_list.append(
            self.gnss_paths[start + 1:start + self.seq_len] - self.gnss_paths[start:start + self.seq_len - 1])
        ...
        # 处理最后一段尾部数据（长度可能不足 seq_len）
        if start + self.seq_len <= len(self.gnss_paths):
            final_gnss_delta = self.gnss_paths[start + 1:start + self.seq_len] - self.gnss_paths[start:start + self.seq_len - 1]
        else:
            final_gnss_delta = self.gnss_paths[start + 1:] - self.gnss_paths[start:-1]
        self.img_paths_list.append(self.img_paths[start:])
        self.gnss_list.append(final_gnss_delta)
    ...
    def __getitem__(self, i):
        ...
        gnss_sequence = torch.FloatTensor(self.gnss_list[i])
        gt_sequence = self.poses_list[i][:, :6]
        return image_sequence, imu_sequence, gnss_sequence, gt_sequence
