...
class PoseTransformer(nn.Module):
    def __init__(self, input_dim=896, embedding_dim=128, num_layers=2, nhead=8, dim_feedforward=512, dropout=0.1):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(input_dim, self.embedding_dim),  # input_dim 这里为 896
        )
        ...
        self.fc2 = nn.Sequential(
            ...
            nn.LeakyReLU(0.1),
            nn.Linear(self.embedding_dim, 6)  # 输出6维
        )
    ...
    def forward(self, batch, gt):
        ...
        # print("！！！！！！！！！！！！！！！Input shape:", visual_inertial_features.shape)
...
class PoseTransformerVisual(nn.Module):
    ...
    def forward(self, batch, gt):
        visual_inertial_features, _, _ = batch
        visual_inertial_features = visual_inertial_features[:,:,:896]  # Ensure visual_inertial_features is 896
        seq_length = visual_inertial_features.size(1)
...
class PoseTransformerInertial(nn.Module):
    ...
    def forward(self, batch, gt):
        visual_inertial_features, _, _ = batch
        visual_inertial_features = visual_inertial_features[:,:,896:]  # Ensure inertial features are 896
        seq_length = visual_inertial_features.size(1)
